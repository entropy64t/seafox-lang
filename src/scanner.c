...
static bool comment() {
	if (peekNext() == '/') {
		...
		advance();
		advance(); // advance past the initial '/*'
		...
		advance();
		advance(); // skip past the trailing '*/'
		return true;
	}
	return false;
}
...
static TokenType identifierType() {
	...
	switch (length) {
		case 2:
			if (memcmp(scanner.start, "if", 2) == 0)
				return TOKEN_IF;
		...
		case 3:
			if (memcmp(scanner.start, "and", 3) == 0)
				return TOKEN_AND;
			if (memcmp(scanner.start, "for", 3) == 0)
				return TOKEN_FOR;
		...
		case 5:
			if (memcmp(scanner.start, "class", 5) == 0)
				return TOKEN_CLASS;
			if (memcmp(scanner.start, "false", 5) == 0)
				return TOKEN_FALSE;
			if (memcmp(scanner.start, "print", 5) == 0)
				return TOKEN_PRINT;
			if (memcmp(scanner.start, "super", 5) == 0)
				return TOKEN_SUPER;
			if (memcmp(scanner.start, "while", 5) == 0)
				return TOKEN_WHILE;
			if (memcmp(scanner.start, "break", 5) == 0)
				return TOKEN_BREAK;
			if (memcmp(scanner.start, "using", 5) == 0)
				return TOKEN_USING;
			if (memcmp(scanner.start, "const", 5) == 0)
				return TOKEN_CONST;
			break;

		case 6:
			if (memcmp(scanner.start, "return", 6) == 0)
				return TOKEN_RETURN;
		...
		case 8:
			if (memcmp(scanner.start, "continue", 8) == 0)
...
#define MAKE_MATCHING_3(e1, e2, m1, m2, other) makeToken(match(e1) ? (m1) : (match(e2) ? (m2) : (other)))
	...
	switch (c) {
		...
		case '-':
			return MAKE_MATCHING_3('-', '=', TOKEN_MINUS_MINUS, TOKEN_MINUS_EQUAL, TOKEN_MINUS);
		case '/':
			return MAKE_MATCHING('=', TOKEN_SLASH_EQUAL, TOKEN_SLASH);
		case '*':
			return MAKE_MATCHING('=', TOKEN_STAR_EQUAL, TOKEN_STAR);
		...
		case '>':
			return MAKE_MATCHING('=', TOKEN_GREATER_EQUAL, TOKEN_GREATER);
